<template>
    <div :class="['mdReader', { mobile: isSmallScreen }]" ref="mdReader">
        <div v-if="isSmallScreen" ref="floatBtn" :class="['float-btn', { light: showFloatBtn }]" @click="drawerVisible = true">
...
import { getNoteContent, getNote } from '@/api/notes';
...
async function renderVditor() {
    ...
    let vditor = new Vditor(previewElement.value, {
        ...
        toolbar: [
            ...
            { name: 'record', tipPosition: 'e' },
            { name: 'edit-mode', tipPosition: 'e' },
            { name: 'both', tipPosition: 'e' },
            { name: 'preview', tipPosition: 'e' },
            { name: 'fullscreen', tipPosition: 'e' },
            { name: 'outline', tipPosition: 'e' },
            { name: 'code-theme', tipPosition: 'e' },
            { name: 'content-theme', tipPosition: 'e' },
            { name: 'export', tipPosition: 'e' },
            { name: 'devtools', tipPosition: 'e' },
            { name: 'info', tipPosition: 'e' },
            { name: 'help', tipPosition: 'e' },
            { name: 'br', tipPosition: 'e' },
        ],
        after() {
            // 触发data-type="preview"的button点击事件
            const previewButton = document.querySelector('[data-type="preview"]') as HTMLButtonElement;
            if (previewButton) {
                previewButton.click();
            }
            // vditor.renderPreview(mdFile); //
        },
    });
...
const initOutline = () => {
    ...
    document.querySelector('.preview').addEventListener('scroll', (el) => {
        const scrollTop = (el.srcElement as HTMLElement).scrollTop || document.body.scrollTop;
        toc = [];
...
.preview {
    ...
    // padding: 1em 2em 1em;
    // background-image: linear-gradient(90deg, rgba(159, 219, 252, 0.3) 3%, transparent 0), linear-gradient(1turn, rgba(159, 219, 252, 0.3) 3%, transparent 0);
    // background-size: 30px 30px;
    // background-position: 50%;
    // background-color: #f9ecdf8e;
}
